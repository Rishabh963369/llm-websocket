...
class GeminiMultimodalProcessor:
    ...
    def __init__(self):
        ...
        self.model = genai.GenerativeModel('gemini-1.5-flash')  # Lightweight multimodal model
...
async def handle_client(websocket):
    ...
    speech_segments_task = None  # Store the speech segments task
    receive_task = None # store the receving audio images task
    try:
        ...
        async def detect_speech_segments():
            nonlocal speech_segments_task  # Allow modification of the outer scope variable
            while True:
                try:
                        ...
                        if "realtime_input" in data:
                            for chunk in data["realtime_input"]["media_chunks"]:
                                if chunk["mime_type"] == "audio/pcm":
                                    ...
                                    # Cancel the speech_segments_task and start with the new input, interrupt
                                    if speech_segments_task:
                                        speech_segments_task.cancel()
                                        try:
                                            await speech_segments_task # Wait for task to cancel
                                        except asyncio.CancelledError:
                                            pass
                                    speech_segments_task = asyncio.create_task(detect_speech_segments()) #start again
        ...
        speech_segments_task = asyncio.create_task(detect_speech_segments())
...
async def main():
    ...
    try:
        ...
        transcriber = WhisperTranscriber.get_instance()
        gemini_processor = GeminiMultimodalProcessor.get_instance()  # Updated
        tts_processor = KokoroTTSProcessor.get_instance()
